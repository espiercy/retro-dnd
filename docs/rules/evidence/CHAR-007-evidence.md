...
> **Completeness-audit correction (2026-08-23).** Audited under proposed `DEC-0010` against **visually verified page images**. One correction applied: the **Abilities and Adjustments Table** was cited throughout as **p. 9**; it is on **p. 10** (p. 9 carries the Bonuses and Penalties table and the Strength write-up). The table's **content is confirmed correct in every row** — only the page reference was wrong, a direct symptom of citing a table from a linearized OCR stream. All four `CHAR-007` governing tables (Bonuses and Penalties p. 9; Abilities and Adjustments, Intelligence and Languages, Charisma Adjustment, all p. 10) and the Ch. 19 mapping (p. 266) are now visually verified. One qualification added: RC p. 9 says high Wisdom gives "a bonus to **one of** his saving throws" — narrower than recorded here. See `docs/rules/evidence/CLUSTER-002-completeness-audit.md`.
>
> **Open-question closure (2026-08-23) — §9 items 1 and 4 are now ANSWERED.** Both named an unread source region; both regions have been visually inspected.
>
> - **§9.1 Open Doors ownership** (Ch. 13 "Doors," p. 147 — previously "not read in full"). RC carries a dedicated **"Open Doors" Ability** section there: 1d6, success on 5–6, **modified by the Strength score adjustment**, natural 6 always opens — and it **adds two rules Ch. 1 omits**: the attempt may be made **once per round per character**, and **a failed attempt forfeits surprise** ("monsters on the other side of the door cannot be surprised; they have heard the noise"). **RESOLVED → CONFIRMED OUT OF SCOPE.** `CHAR-007` owns the Strength **adjustment value**; the **procedure** belongs to `EXP-005`, where p. 147 sits alongside Doors, Secret Doors, Special Doors and Listening. New dependency: `ENC-002` (surprise).
> - **§9.4 ability effect on initiative** (Ch. 8, previously "not exhaustively searched"). RC p. 102 carries **"Dexterity Adjustments (Optional)"**: at DM option Dexterity modifies the **individual** initiative roll, and explicitly "does not affect the party's roll in group initiative." Individual initiative is itself optional and is **declined for V1** by `DEC-0008`. **RESOLVED → CONFIRMED OUT OF SCOPE** (owner `COMBAT-006`). This packet's V1 effect list is correct. RC's cross-reference there cites the adjustment table as "on **page 9**", independently confirming the p. 9 / p. 10 split corrected above.
>
> §9 items 2 (Armor Class → `COMBAT-002`) and 3 (Charisma retainer columns → `CHAR-006`) are likewise **CONFIRMED OUT OF SCOPE**; item 5 (Wisdom overlap) is preserved per human ruling. See `docs/rules/evidence/CLUSTER-002-completeness-audit.md` §5.4 and §6.3.
>
