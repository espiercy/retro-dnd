...
| A. Reference point for cadence | Ch. 7 p. 91, Game Turn Checklist (the check is step 4 of each iteration) | The two-turn cadence is counted in Game-Turn-Checklist iterations, not an absolute elapsed-minutes clock separate from that procedure. | Rules Cyclopedia Explicit | DIRECT PRIMARY TEXT |
...
This procedure answers, RC-natively: *what* triggers a check, *how often*, *what die/result*, *that trigger and arrival are distinct events one turn apart*, and *that the check does not run during round-mode*. It does **not** answer whether an encounter's own credited turn(s) — per `EXP-002`'s now-approved discrete whole-turn-credit model — participate in the every-two-turns count when normal play resumes.
...
**Tentative conclusion (cadence):** the dungeon wandering-monster check is made once every two Game-Turn-Checklist iterations, never every single turn as a baseline rule.
...
**Disposition:** **CONFIRMED** (baseline), with the noise/battle/etc. modifier recorded as a separate, explicitly-flagged exception (see Evidence Map, row G).
...
**Evidence:** every located passage is mutually consistent — checklist step 1 ("check at the end of the previous turn... arrive now"), checklist step 4 ("at the beginning of the next turn"), and the subsection ("they appear the following turn") all describe the same one-turn-delayed arrival with no located exception.
**Disposition:** **CONFIRMED**.
...
- **RC establishes**, independently and consistently with the above: the wandering-monster check is a Game-Turn-Checklist step; it does not run during round-mode (Encounter Checklist explicitly switches game time to rounds); a positive check delays arrival by exactly one turn.
