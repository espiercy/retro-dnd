...
| `EXP-002` | Dungeon Turn / Time Accounting | Vol. 3, p. 8 ("THE MOVE/TURN IN THE UNDERWORLD") | — | Yes | **APPROVED** | **High priority** — `EXP-001` already depends on this (its own Open Questions item). Partial text already extracted: a turn ≈ 10 minutes ≈ two moves; resting consumes one full turn per hour; searching a 10' wall section takes a full turn, shorter activities a referee-adjudged portion; combat is ten rounds per turn. The precise accounting *algorithm* for a computer simulation (how partial-turn activities accumulate, exact check-firing timing) is not yet specified. |
