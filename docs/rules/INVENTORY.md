...
1. **`EXP-001` is materially changed, not merely uncertain.** Primary-text review establishes the Rules Cyclopedia's dungeon wandering-monster procedure as: check every *other* dungeon turn; roll 1d6; on a 1, wandering monsters appear at the *beginning of the next turn* (not immediately). This differs from the old 1974-derived Rule Card's "every turn, immediate appearance" specification in both cadence and timing. `EXP-001` remains `REVALIDATION_REQUIRED`; the Rule Card itself is not rewritten this task — see "Treatment of Existing Rule Cards" in `INVENTORY_MIGRATION_MAP.md`.
...
| ID | Title | RC Source | RC Classification / Project Selection | Dependencies | Downstream Consumers | V1 Reachable | Status | Risk / Notes |
|---|---|---|---|---|---|---|---|---|
| `EXP-001` | Dungeon Wandering-Monster Check | Dungeon Adventures chapter | RC Core | RNG abstraction, `EXP-002` | — | Yes | **`REVALIDATION_REQUIRED`** (existing Rule Card, not revalidated this task) | **High, materially changed** — every-*other*-turn cadence, 1d6 with encounter on a 1, appearance at the *beginning of the next turn* (not immediate). See Major Research-Risk Flags item 1. |
| `EXP-002` | Dungeon Turn / Time Accounting | Dungeon Adventures chapter | RC Core | — | `EXP-001`, `EXP-003`–`EXP-010`, `EXP-004` | Yes | **`REVALIDATION_REQUIRED`** (existing Rule Card, not revalidated this task) | Medium — the broad ten-minute-turn responsibility appears to survive, but the old accumulator's every-turn `EXP-001` integration contract cannot be assumed correct now that `EXP-001`'s own cadence has changed; revalidation must reassess the integration relationship directly, not just the turn-length fact. |
