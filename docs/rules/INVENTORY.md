...
|---|---|---|---|---|---|---|---|---|
| `CHAR-001` | Ability Score Generation | Character Creation, Ch. 1 (pp. 6–7); **Ch. 10 p. 130** (above-1st-level methods); **Ch. 13 p. 145** (DM discard/switch provisions) | RC Core | — | `CHAR-002`, `CHAR-003`, `CHAR-007` | Yes | **Stage-B drafted — `AWAITING_APPROVAL`** (`docs/rules/character_creation/ability_score_generation.md`) | Low for the baseline 3d6 procedure. **Source locations corrected 2026-08-29** — this responsibility is materially larger than "Chapter 1": Ch. 1 also carries the 2-for-1 prime-requisite trade and its four constraints; Ch. 13 p. 145 adds DM-discretionary discard and score-switch provisions; Ch. 10 p. 130 adds two above-1st-level methods, **one of them non-random**. Four narrow RC ambiguities are carried as human decisions on the card, each with a guard test. |
