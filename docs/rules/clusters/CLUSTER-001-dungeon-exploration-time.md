...
> **This section is historical and does not describe the current cluster.** Everything from here to the end of this document is the original 1974-primary-sourced cluster proposal, approved 2026-08-15 under the since-superseded 1974-primary source policy (`DEC-0006-v1-playable-content-scope.md`, superseded by `DEC-0007`), together with the 2026-08-16 migration note that recorded its suspension pending Rules Cyclopedia revalidation. It is preserved verbatim below for provenance — to show the reasoning and research that led to today's re-derived boundary above — not as a statement of `CLUSTER-001`'s current scope, membership, or status. In particular: **do not read anything below as saying `EXP-004` is part of the current cluster, that a mandatory-hourly-rest mechanic is current Rules Cyclopedia canon, or that `EXP-001`'s "already `APPROVED`" language reflects its current lifecycle status** — `EXP-001` is `REVALIDATION_REQUIRED`, per the Current section above.
