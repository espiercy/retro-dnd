...
**Further outcome (2026-08-23), recorded in the same spirit as the note above.** Two further items from this analysis have since been acted upon by human decision, and are no longer open: (a) §7's runner-up, **Character-Foundation**, has been selected and boundary-approved as `CLUSTER-002` — see `docs/rules/clusters/CLUSTER-002-character-foundation.md`; (b) §5.2's flagged `CHAR-007` `SPLIT CANDIDATE` risk has been **resolved** by narrowing `CHAR-007` to general ability-score mechanical effects and assigning the Chapter 19 ability-to-saving-throw mapping solely to `COMBAT-004`. §5.2's own statement that `INVENTORY.md` "is not modified" was accurate when written and remains the correct account of *this* task's scope; that inventory change was made later, by the separate 2026-08-23 boundary approval. The analysis body below is preserved unchanged as historical record and is not rewritten to reflect either outcome.
