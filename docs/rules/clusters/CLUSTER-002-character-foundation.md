...
| **Human rulings (2026-08-29)** | **SR-1** Elf fixed HP at 10th = **+2**; **SR-2** a Mystic **may** raise Dexterity through the 2-for-1 trade; **SR-3** the Ch. 13 score switch is **included in V1**, runs **before** eligibility and may establish a class minimum, while the trade runs after and never can; **SR-4** discard offered when no score is above 9 **or** two are below 6, player may keep anyway. **All four are `Simulator Ruling` provenance — project adjudications, not RC findings** |
| Rule Card approval (`CHAR-001`, `CHAR-002`, `CHAR-003`, `CHAR-007`) | **Not granted — all four `AWAITING_APPROVAL`**, and all four now **mechanically complete and ready for review**. Approving `CHAR-003` ratifies SR-1; approving `CHAR-001` ratifies SR-2, SR-3 and SR-4 |
| Alternate-source governance | **`DEC-0011` `PROPOSED — AWAITING HUMAN APPROVAL`**; `RULE_CARD_RESEARCH_PROTOCOL.md` §15.1 drafted and **NOT IN FORCE** |
