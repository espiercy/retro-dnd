...
5. **Smallest proposed ruling.** `turns_since_last_check` is a single shared counter for both normal and heightened checking. It resets to 0 whenever a due ordinary check is resolved — performed or skipped (per the separately RC-Explicit "DM-discretion skip input") — whether that due-ness arose from the normal two-turn threshold or from heightened checking's every-turn condition; heightened checking's activation or deactivation does not, by itself, reset or otherwise modify the counter. This requires no additional state (no saved phase, no independent counters) and is the direct, unmodified continuation of arithmetic the card already needs for ordinary cadence tracking.
...
**Clarification (2026-08-18, approval-preserving, not a reopening of this ruling).** Ruling C governs the heightened-checking *transition* question only — it establishes that activating or deactivating heightened checking, by itself, never resets the shared tally. It does not narrow or supersede the separately RC-Explicit, separately approved pre-decided-period skip's own reset effect ("Rules Cyclopedia Explicit" item 6, "DM-discretion skip input," "Approved Mechanical Specification"): a due ordinary check's period is consumed, and the tally resets, whether that check is performed or skipped. Both facts hold simultaneously and are not in tension — Ruling C addresses *what heightened-mode transitions do*; the skip provision addresses *what resolving a due period does*.
...
turns_since_last_check : integer, starts at 0
                          ...
                          (Simulator Ruling A). Resets to 0 when an
                          ordinary due period is resolved: either by a
                          performed wandering-monster check, or by the
                          approved pre-decided-period skip (Rules
                          Cyclopedia Explicit item 6, "DM-discretion
                          skip input" below) — both consume the due
                          period identically. Activating or deactivating
                          heightened checking alone never resets or
                          otherwise modifies it (Simulator Ruling C).
...
**Heightened-checking input.** Per Rules Cyclopedia Explicit item 7 (corrected reading), this card accepts two independent externally supplied inputs when active: (a) a frequency flag (`heightened_checking`), and (b) a chance level among {1-in-6, 2-in-6, 3-in-6, 4-in-6}. **When heightened checking is active, the cadence becomes due after each credited turn rather than every two credits.** Consistent with Simulator Rulings A and B, this changes only the due threshold, not where a check may execute: actual check execution remains limited to an ordinary Game-Turn-Checklist step-4 opportunity, and an encounter-derived credit may make or keep the cadence due but does not itself execute a check. Per **Simulator Ruling C**, `heightened_checking` becoming active or inactive does not, by itself, reset or otherwise adjust `turns_since_last_check` — the same shared counter continues uninterrupted, and only the resolution of a due ordinary check (Procedure A's due branch, on an ordinary credit — whether the roll is performed or skipped per the "DM-discretion skip input" above) resets it, regardless of which threshold made it due. **This card does not decide which upstream condition sets these inputs, or which level applies** — that determination belongs to whichever system asserts a loud-noise/battle/cursed-item/special-area condition (see "Open Questions"). Absent an explicit input, both default to the ordinary baseline (every two turns, 1-in-6).
