...
| `CHAR-007` | Ability Score Mechanical Effects & Cross-System Dependencies | SPLIT CANDIDATE, executed narrowly (2026-08-23) | `CHAR-007` (general ability-score mechanical effects) + `COMBAT-004` (Chapter 19 ability-to-saving-throw mapping) | Broad responsibility retained under `CHAR-007`; RC's specific modifier table is expected to differ numerically. Narrowed at `CLUSTER-002` boundary approval: the Chapter 19 ability-modifier-to-saving-throw mapping moved to `COMBAT-004`, which cannot be specified without that card's own five save categories. Same principle as the `CHAR-003` row above — narrow the `CHAR` entry rather than duplicate saving-throw ownership in two places; no new `CHAR-*` ID created. Executed before research, so no approved specification or lifecycle metadata was disturbed. See `docs/rules/clusters/CLUSTER-002-character-foundation.md`. |
...
| `COMBAT-004` | Saving Throws | RETAIN/REVALIDATE | `COMBAT-004` | Same responsibility; now also absorbs `CHAR-003`'s old saving-throw half (see `CHAR-003` above) and, as of 2026-08-23, sole ownership of the selected Chapter 19 ability-modifier-to-saving-throw mapping formerly also claimed by `CHAR-007` (see `CHAR-007` above). Five-category structure corroborated. |
