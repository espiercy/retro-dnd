...
- `docs/rules/RULE_CARD_RESEARCH_PROTOCOL.md` governs *how Rule Card research and revalidation is performed* — the Evidence-First two-stage process (`DEC-0009-evidence-first-rule-research-protocol.md`). §9.7 below defers to it rather than restating it.
...
Revalidation follows the **Evidence-First** two-stage protocol required for all substantial Rule Card research (`docs/rules/RULE_CARD_RESEARCH_PROTOCOL.md`, adopted by `DEC-0009-evidence-first-rule-research-protocol.md`), not the older single research→draft→human-review pass this section previously described. In substance: an **Evidence stage** — researching the current source hierarchy's governing procedure from scratch, independently of the superseded specification, followed by a whole-source cross-reference pass and a falsification pass — then a **human evidence-review gate**, then a **Synthesis/Rule Card Draft stage** that only now classifies each inherited legacy mechanic (PRESERVED / CHANGED / REMOVED / MOVED TO ANOTHER RESPONSIBILITY / RC DOES NOT SPECIFY / POTENTIAL COMPLETION QUESTION) against the independently-established current procedure, then **human Rule Card approval**. It may conclude either that the prior specification survives essentially unchanged (with updated provenance labeling) or that it requires material rewriting and new deterministic tests. No artifact may be silently grandfathered back to `APPROVED` merely because its previously-researched mechanics happen to resemble the current authority's rule — revalidation must be performed and recorded, not assumed, and must not anchor on the superseded Rule Card's own framing before the current authority's procedure is independently understood.
